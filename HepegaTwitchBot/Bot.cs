...
namespace HepegaTwitchBot
{
    public class Bot
    {
        private int EASY_COMMAND = 3, MEDIUM_COMMAND = 5, HIGH_COMMAND = 7, SPAM_COMMAND = 7, AI_COMMAND = 4;
        TwitchClient client;
        ...
        private int easyCommand = 0;
        private int mediumCommand = 0; 
        private int highCommand = 0;
        private int spamCommand = 0;
        private int aiCommand = 0;
        private bool spamAllowed = false;
        private int spamSymbolCount = 500;
        ...
        {
            Console.WriteLine("Привет! Я неофициальный бот HPG 2.0. Пиши !команды в чат, чтобы узнать список доступных команд.");
        ...
        private void Client_OnMessageReceived(object sender, OnMessageReceivedArgs e)
        {
            ...
            if (message.StartsWith("!команды") && easyCommand == 0)
            {
                ...
                client.SendMessage(channel, $"@{username} Список доступных команд: !топхпг, !хпгигра [ник_участника], !события [ник_участника], !шмотка [название_шмотки_или_события], !hltb [название_игры], !gamefaq [название_игры], !коронавирус [название_страны_на_английском]");
            }
            else if (message.StartsWith("!топхпг") && mediumCommand == 0)
            {
                mediumCommand = MEDIUM_COMMAND;
                string topHpg = hpgDoc.GetLeaderboard();
                client.SendMessage(channel, $"@{username} {topHpg}");
            }
            ...
            else if (message[0] != '@' &&  message[0] != '!' && !username.Contains("bot") && username != "streamelements" && !message.Contains("http"))
            {
                string substring = message.Substring(0, message.Length >= 10 ? 10 : message.Length);
                ...
                if (messages[substring] > (random.Next(6,10)))
                {
        ...
        private void SendItemMessage(string message, string channel, string username)
        {
            ...
            {
                string result = items.Where(s => s.ToLower().Contains(message.ToLower())).ToArray()[0];
        ...
        private async void SendHltbMessage(string username, string game, string channel)
        {
            string result;
            string nickname = HasANickname(game);
            if (nickname != "null")
            {
            ...
            {
                result = await hltbParser.ParseGame(game);
            }

            client.SendMessage(channel, $"@{username} {result}");
        }

        private async void SendGamefaqMessage(string username, string game, string channel)
        {
            string result;
            string nickname = HasANickname(game);
            if (nickname != "null")
            ...
            {
                result = await gamefaqParser.ParseGame(game);
            }
        ...
        {
            string result;
            string nickname = HasANickname(message);
            if (nickname != "null")
            {
                ParticipantInfo participantInfo = hpgDoc.GetParticipantInfo(nickname);
        ...
        private string HasANickname(string message)
        {
            string result;
        ...
        {
            client.SendWhisper(e.WhisperMessage.Username, "Привет! Если хочешь пообщаться с создателем бота - пиши \"/w PDVRR <сообщение>\" в чате!");
        }
    }
}